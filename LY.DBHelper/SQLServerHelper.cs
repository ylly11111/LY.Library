...
namespace LY.DBHelper
{
    ...
    public sealed class SqlServerHelper
    {
        ...
        /// 连接信息xml对象
        /// </summary>
        private static MSqlServerConnectionInfo m_ConnectionInfo = null;
        /// <summary>
        /// 连接信息xml对象
        /// </summary>
        public static MSqlServerConnectionInfo ConnectionInfo
        {
        ...
        /// 根据连接信息对象生成连接字符串
        /// </summary>
        /// <param name="info">连接信息对象</param>
        /// <returns></returns>
        public static string GetConnectionString(MSqlServerConnectionInfo info)
        {
            return string.Format(@"Data Source={0};UID={1};pwd={2};Initial Catalog={3};Connect Timeout={4}", info.DataSource, info.UserID, info.Password, info.DataBaseName, info.ConnectTimeout);
        }
        ...
        /// <returns></returns>
        public static bool TestConnection(MSqlServerConnectionInfo info)
        {
            bool tmpRet = false;
            string tmpConnectionString = string.Format(@"Data Source={0};UID={1};pwd={2};Initial Catalog={3};Connect Timeout={4}", info.DataSource, info.UserID, info.Password, info.DataBaseName, 1);
        ...
        {
            if (m_ConnectionInfo == null) throw new ArgumentNullException("ConnectionInfo对象无效");
            return ExecuteNonQuery(GetConnectionString(m_ConnectionInfo), commandType, commandText, (SqlParameter[])null);
        }
        ...
        {
            if (m_ConnectionInfo == null) throw new ArgumentNullException("ConnectionInfo对象无效");
            return ExecuteNonQuery(GetConnectionString(m_ConnectionInfo), commandType, commandText, commandParameters);
        }
        ...
        {
            if (m_ConnectionInfo == null) throw new ArgumentNullException("ConnectionInfo对象无效");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName无效");
            // 如果存在参数值
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                // 从探索存储过程参数(加载到缓存)并分配给存储过程参数数组.
                SqlParameter[] commandParameters = SqlHelperParameterCache.GetSpParameterSet(GetConnectionString(m_ConnectionInfo), spName);
                // 给存储过程参数赋值
                AssignParameterValues(commandParameters, parameterValues);
                return ExecuteNonQuery(GetConnectionString(m_ConnectionInfo), CommandType.StoredProcedure, spName, commandParameters);
            }
            else
            {
                // 没有参数情况下
                return ExecuteNonQuery(GetConnectionString(m_ConnectionInfo), CommandType.StoredProcedure, spName);
            }
        }
        ...
        {
            if (m_ConnectionInfo == null) throw new ArgumentNullException("ConnectionInfo对象无效");
            using (SqlConnection connection = new SqlConnection(GetConnectionString(m_ConnectionInfo)))
            {
                connection.Open();
                // 调用指定数据库连接字符串重载方法.
        ...
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString无效");
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                return ExecuteDataset(connection, commandType, commandText, (SqlParameter[])null);
            }
        ...
        public static object ExecuteScalar(CommandType commandType, string commandText)
        {
            // 执行参数为空的方法
            if (m_ConnectionInfo == null) throw new ArgumentNullException("ConnectionInfo对象无效");
            return ExecuteScalar(GetConnectionString(m_ConnectionInfo), commandType, commandText, (SqlParameter[])null);
        }
        ...
        public static object ExecuteScalar(CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            // 执行参数为空的方法
            if (m_ConnectionInfo == null) throw new ArgumentNullException("ConnectionInfo对象无效");
            return ExecuteScalar(GetConnectionString(m_ConnectionInfo), commandType, commandText, commandParameters);
        }
    ...
    public sealed class SqlHelperParameterCache
    {
        ...
        public static SqlParameter[] GetSpParameterSet(string connectionString, string spName, bool includeReturnValueParameter)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    //LogSaver.WriteLog(string.Format("调用 GetSpParameterSet 出错：｛0｝", ex.ToString()));
                    return null;
                }
    ...
    /// <summary>
    /// sqlserver的连接信息Model类
    /// </summary>
    [XmlRoot("SqlServerConnectionInfo")]
    public class MSqlServerConnectionInfo
    {
        ...
        public int ConnectTimeout = 1;//缩短测试连接的时间
    }
}
